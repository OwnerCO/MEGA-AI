...
function startBot() {
  ...
  if (!phoneNumber) {
    console.warn(chalk.yellow('BOT_NUMBER environment variable is not set. You may need to enter it manually.'))
  }

  const currentFilePath = new URL(import.meta.url).pathname
  const args = [path.join(path.dirname(currentFilePath), 'global.js'), ...process.argv.slice(2)]
